...
Open app in browser
Tap install banner at bottom ("Install App") or menu → "Install app"
App appears on home screen; opens in standalone mode
On iOS (Safari):

Open app in Safari
Tap Share → "Add to Home Screen"
App appears on home screen with custom icon and name
Offline Support:
...
Equipment Registry
Add Equipment: Click "Add Equipment" → fill form with details → save
View Details: Click equipment row → view full record with attachments and scoring breakdown
Edit Equipment: Click "Edit" → modify fields → save changes
Delete Equipment: Use detail view delete button (requires confirmation)
...
Make changes and test locally: npm run dev
Build and verify: npm run build
Commit with descriptive message: git commit -m "Add: brief description"
...
Default settings work; no env vars needed
Deploy → app live at yourdomain.vercel.app
Auto-deployment: Every push to main triggers production build

Manual Deployment (any static host)
Build locally: npm run build
Upload dist/ folder to your server (Netlify, GitHub Pages, AWS S3, etc.)
...
ASHE Guidelines: Association for the Advancement of Medical Instrumentation
ECRI Institute: Equipment Lifecycle & Reliability Data
FDA Safety: Medical Device Recalls & Alerts
